...
##### 📝 仓库名单填写格式说明（黑白名单通用）：
- **短名与全名皆可**：
  - **短名（推荐，最省事）**：如 `my-tool`（无需写用户名前缀）。
  - **全名（带用户名）**：如 `your_username/my-tool`。
  - *注：系统自动支持两种写法，且自动忽略大小写（如 `My-Tool` 和 `my-tool` 等效）。*
- **分隔符完全兼容（空格随意、纯空格也支持）**：
  - **逗号+空格（推荐）**：`repo1, repo2, owner/repo3`
  - **纯逗号（紧凑）**：`repo1,repo2,owner/repo3`
  - **纯空格分隔**：`repo1 repo2 owner/repo3`
  - **分号分隔**：`repo1; repo2; owner/repo3`
  - **多行换行**：支持直接多行粘贴。
  - *注：无论写不写空格、写几个空格，系统都会自动去除首尾多余空白，精准匹配。*
...
- `{failed}`：异常失败数
- `{issues}`：**问题仓库与分支完整明细**（包含具体仓库名、分支名与跳过/失败原因）
- `{issue_repos}`：**仅提示问题仓库名（多行列表）**，不输出分支名与报错文本（如 `- \`xb18/repo1\``）
- `{issue_repos_inline}`：**仅提示问题仓库名（单行逗号隔开）**（如 `\`xb18/repo1\`, \`xb18/repo2\``）
- `{warnings}`：安全跳过/分叉保护的分支完整明细
- `{errors}`：异常失败的分支完整明细
